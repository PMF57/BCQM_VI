...
if bool(abl.get('glue_decohere', False)):
    # Disable phase lock and cadence coupling (keep other params for provenance).
    try:
        phase.enabled = False
    except Exception:
        pass
    ...
    try:
        ...
        pass
    # Disable shared bias and domains (conservative).
    try:
...
if hasattr(cfg_hop, 'q_base_override'):
    ...
    # Geometry diagnostics (spectral dimension) — optional
    geom_cfg = cfg.get("geometry", {}) or {}
    geom_out = None
    if bool(geom_cfg.get("enabled", False)) and space["enabled"]:
        assert g is not None
        try:
            geom_out = g.estimate_spectral_dimension(
                active_end,
                walkers=int(geom_cfg.get("walkers", 200)),
                steps=int(geom_cfg.get("steps", 4000)),
                fit_t_min=int(geom_cfg.get("fit_t_min", 30)),
                fit_t_max=int(geom_cfg.get("fit_t_max", 400)),
                seed=int(seed),
            )
        except Exception as e:
            geom_out = {"ds_est": None, "ds_valid": False, "notes": f"exception:{type(e).__name__}"}
