...
"""
engine_vglue.py (BCQM VI) [ball-growth diagnostic v0.1]
...
def run_single_v_glue(cfg: Dict[str, Any], N: int, n: float, seed: int) -> None:
    ...
    if space["enabled"]:
        ...
        geometry_out = {'enabled': geom_enabled}
        if geom_enabled:
        ...
        # Diagnostic: ball-growth profile |B(r)| vs r on largest component (helps interpret ds failures).
        try:
            geometry_out["ball_growth"] = g.ball_growth_profile(
                active_end,
                r_max=int(geom_cfg.get("ball_r_max", 25)),
                samples=int(geom_cfg.get("ball_samples", 30)),
                seed=int(seed) + 54321,
            )
