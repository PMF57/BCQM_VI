#!/usr/bin/env python3
"""
timeseries_ensemble_summary.py (v0.1.1)

Fix:
- Handle DataFrame selection correctly when extracting Fmax_0.20 (avoid Series->float TypeError).
...
def load_metrics(folder: Path):
    ...
    mets = []
    for f in files:
        mets.append(json.loads(f.read_text(encoding="utf-8")))
    return mets
    ...
    for tag, rel in ROOTS:
        ...
        for t in tvals:
            ...
            for df in dfs:
                ...
                # Fix: sub.get(...) returns a Series; use iloc[0] if column exists.
                if "Fmax_0.20" in sub.columns:
                    vals_F.append(float(sub["Fmax_0.20"].iloc[0]))
                else:
