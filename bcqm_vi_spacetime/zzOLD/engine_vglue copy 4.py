...
def _space_cfg(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "p_reuse": sp.get("p_reuse", None),  # if None, derived from R*active_mean
...
def _derive_p_reuse(space: Dict[str, Any], threads: ThreadState) -> float:
    if space["p_reuse"] is not None:
        return float(space["p_reuse"])
    # Derived heuristic (logged): reuse increases with phase coherence and cadence activity.
...
def run_single_v_glue(cfg: Dict[str, Any], N: int, n: float, seed: int) -> None:
    ...
    for t in range(steps_total):
        ...
        if space["enabled"]:
            ...
            p_reuse = _derive_p_reuse(space, threads)
    ...
    if space["enabled"]:
        ...
        space_out.update({
            ...
            "p_reuse_policy": "derived_from_R_active" if space["p_reuse"] is None else "fixed",
            "p_reuse_value_last": float(_derive_p_reuse(space, threads)),
