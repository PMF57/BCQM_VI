...
def _resolve_snapshot_epochs(cfg: Dict[str, Any]) -> List[int]:
    """Resolve snapshot epochs.

    Policy: if snapshots are enabled, we still write a dedicated *_final snapshot.
    Therefore, we exclude the final epoch from cadence/explicit epoch lists here to
    avoid duplicate content with different filenames.
    """
    snaps = cfg["snapshots"]
...
def _choose_preferred_existing(
    ...
    """Choose a global 'channel' target (preferred existing event) for tailgating.

    Critical guard-rail (quickcheck fix):
      - DO NOT choose by max indegree (causes rich-get-richer star collapse).
      - Prefer *recent* and *not-already-hub* nodes.
    """
    if not active:
        return None

    # Prefer not to target current frontier nodes when possible (avoids self-loop stalling).
    frontier_set = set(frontier)
    pool = [v for v in active if v not in frontier_set] or active

    W_coh = int(cfg.get("W_coh", 256))
    tau = max(1, W_coh // 4)          # recency scale
    alpha = 0.75                      # anti-hub penalty exponent

    def score(vid: int) -> float:
        nd = g.nodes[vid]
        age = max(0, epoch - int(nd.created_at))
        # recency weight * anti-hub weight
        return math.exp(-age / float(tau)) / ((1.0 + float(nd.indeg)) ** float(alpha))

    # Pick from top-k scored nodes (adds mild diversity; still reproducible via rng).
    k = min(10, len(pool))
...
def run_single(cfg: Dict[str, Any], N: int, n: float, seed: int) -> None:
    ...
    lockstep_only_policy = cfg["crosslinks"].get("lockstep_only_policy", "force_new")
    _ = lockstep_only_policy  # reserved; v0.1 uses force_new fallback
    new_event_policy = cfg["crosslinks"]["new_event_policy"]
    _ = new_event_policy  # v0.1 supports only unique_per_thread
    ...
    for epoch in range(1, steps_total + 1):
        ...
        for i, (kind, target) in enumerate(choices):
            ...
            # Guard-rail: disallow self-loops (prevents freezing / infinite self-loop collapse).
            if v == u:
            ...
            if (not allow_junctions) and (g.nodes[v].indeg >= 1):
                # Would create a junction in lockstep-only => force new.
                v2 = g.new_node(created_at=epoch)
                g.add_edge(u, v2, w=1.0, epoch=epoch)
                frontier[i] = v2
            else:
    ...
    if cfg["snapshots"].get("enabled", False):
        write_edges_csv(out_dir / f"SNAPSHOT_{run_id}_edges_final.csv", g.edges)
        write_nodes_json(out_dir / f"SNAPSHOT_{run_id}_nodes_final.json", g)
