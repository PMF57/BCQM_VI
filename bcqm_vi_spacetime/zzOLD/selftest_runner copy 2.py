...
def main() -> None:
    ...
    def _validate_all_yamls(cfg_root: Path) -> list[str]:
        bad = []
    ...
    bad = _validate_all_yamls(root / "configs")
    if bad:
        log.write_text(log.read_text(encoding="utf-8") + "== YAML VALIDATION FAILURES ==\n" + "\n".join(bad) + "\n", encoding="utf-8")
        raise RuntimeError(f"YAML validation failed for {len(bad)} file(s). See log for details.")
    log.write_text(log.read_text(encoding="utf-8") + "OK: all YAML configs under configs/ validate()\\n\\n", encoding="utf-8")
    configs = [
